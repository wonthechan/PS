 ...
 */
public class Main_B17135_캐슬디펜스_bitmasking {
	...
	/** bitmasking 을 사용하여 visit 관리 */
	private static void generateCombo(int idx, int level, int selected) {
		/* 궁수의 범위는 (N+1, 0~M-1) */
		if (level == 3) { // 기저조건
...
//			System.out.println(Arrays.toString(archeryX));
		...
		for (int i = idx; i < M; i++) {
			generateCombo(i + 1, level + 1, selected | 1 << i);
		}
	}
